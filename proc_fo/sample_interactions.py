...
def main(args):
  if args.interactions_file is not None:
    raise Exception("Unplausible. interactions_sampler can only sample interactions file that you just provided...!")
  ...
  try:
    with open(Path(args.save_dir, 'user_interactions.tsv'), 'w') as _out:
      print(f"wisski_user\twisski_item\tat", file=_out)
      logger.info("sampling KG-informed profiles now")
      n_already_sampled = prepare_and_sample_from_kg(args, n_already_sampled, _out)
      logger.info("sampling KG-informed profiles done")
  except:
    Path(args.save_dir, 'user_interactions.tsv').unlink(missing_ok=True)
    logger.warning(f"an error occurred. removed user_interactions.tsv")
    return
