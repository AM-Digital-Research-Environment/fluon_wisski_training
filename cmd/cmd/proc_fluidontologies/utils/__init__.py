...
def plausible_checks_for_args(args, sample_real_profiles, n_real_profiles):
    ...
    if args.use_kg_sampling:
        ...
        if (args.n_rand + args.n_within_range + args.n_along_path) < N_LEFT:
            args.n_rand += N_LEFT - (
                args.n_rand + args.n_within_range + args.n_along_path
            )
    return args
...
def get_preconfigured_parser():
    ...
    parser.add_argument(
        "--log_to_file", action="store_true", help="Redirect log to file"
    )

    parser.add_argument(
        "--log_to_console_off", action="store_true", help="Turn off console output"
    )

    parser.add_argument(
        "--log_dir", nargs="?", default="logs", help="choose logging directory"
    )

    parser.add_argument("--items_file", nargs="?", help="items file")
    parser.add_argument("--entities_file", nargs="?", help="entities file")
    parser.add_argument(
        "--knowledge_graph_file", nargs="?", help="file containing knowledge graph"
    )
    parser.add_argument(
        "--user_file", default=None, nargs="?", help="file containing user ids"
    )
    parser.add_argument(
        "--interactions_file",
        default=None,
        nargs="?",
        help="file containing interactions of real users",
    )
    parser.add_argument(
        "--save_dir", default=".", nargs="?", help="save numpy objects to this dir"
    )
