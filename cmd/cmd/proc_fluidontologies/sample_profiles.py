...
from .utils import sample_test_train, plausible_checks_for_args
            ...
            for item in items:
                ...
                if len(ids) > 0:
                    ...
                    if train is not False and test is not False:
                        only_zeros = False
                        print(
                            f"{i} {' '.join([str(j) for j in train])}",
                            file=outfile_train,
                        )
                        print(
                            f"{i} {' '.join([str(j) for j in test])}", file=outfile_test
                        )
                        n_already_sampled += 1
                        break
    ...
    if args.n_rand > 0:
        ...
        for i in range(n_already_sampled, args.n_rand + n_already_sampled):
            ...
            if len(items) > args.n_interact_min:
                ...
                if train is not False and test is not False:
                    print(
                        f"{i} {' '.join([str(j) for j in train])}", file=outfile_train
                    )
...
def main(args):
    ...
    try:
        with open(Path(args.save_dir, "train.txt"), "w") as _train, open(
            Path(args.save_dir, "test.txt"), "w"
        ) as _test:
            ...
            if args.use_kg_sampling:
                ...
                n_already_sampled = prepare_and_sample_from_kg(
                    args, n_already_sampled, _train, _test
                )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=100,
        type=int,
        help="number of profiles to sample in total",
    )
    parser.add_argument(
        "--perc_within_range",
        default=50.0,
        type=float,
        help="percentage of profiles that will be sampled from network proximity of random items. if perc_within_range and perc_along_path do not add up to 100%, the rest will be sampled randomly",
    )
    parser.add_argument(
        "--perc_along_path",
        default=50.0,
        type=float,
        help="percentage of profiles that will be sampled from network paths of items. if perc_within_range and perc_along_path do not add up to 100%, the rest will be sampled randomly",
    )
    ...
    parser.add_argument(
        ...
        default=200,
        type=int,
        help="maximum number of interactions per sampled profile",
    )
    parser.add_argument(
        "--n_interact_min",
        default=30,
        type=int,
        help="minimal number of interactions per sampled profile",
    )
    parser.add_argument(
        "--n_interact_test_max",
        default=10,
        type=int,
        help="maximum number of interactions per sampled profile that go to the test set",
    )
